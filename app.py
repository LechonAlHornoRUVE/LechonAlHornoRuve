...
def ticket(id):
    ...
    return render_template_string(STYLE+f'<div class="container" style="max-width:380px;margin-top:20px"><div class="card" style="background:white;color:black;border:2px dashed black"><div class="text-center"><img src="/static/logo.png?v=ruve3" style="width:110px"><h5 style="font-weight:bold">LECHÓN AL HORNO RUVE</h5><small>EL SABOR HACE LA DIFERENCIA</small></div><hr style="border-top:1px dashed black"><p><b>Ticket #{v.id}</b><br>Cliente: {v.cliente}<br>Fecha: {v.fecha.strftime("%d/%m/%Y %H:%M")}<br>Producto: {v.producto_nombre}<br>Cant: {v.cantidad}<br><b>Total: ${v.total}</b></p><hr style="border-top:1px dashed black"><p class="text-center" style="font-size:12px">¡Gracias por su compra! 🔥<br>Chetumal, Q. Roo</p><div class="text-center no-print"><button onclick="window.print()" class="btn-rosa">🖨️ IMPRIMIR</button>'+(f'<a href="https://wa.me/?text={msj}" target="_blank" class="btn-whats ms-2 p-2">📲 Enviar WhatsApp</a>' if cfg.whatsapp_btn else '')+f'<a href="/dashboard" class="btn btn-dark ms-2">Volver</a></div></div></div>')
...
def reporte():
    ...
    return render_template_string(STYLE+nav()+'<div class="container mt-4"><div class="card"><h4>Total Hoy: <span style="color:#25D366">${{total_hoy}}</span> ({{ventas_hoy|length}} ventas)</h4><div class="no-print mt-3"><button onclick="window.print()" class="btn-rosa me-2">🖨️ IMPRIMIR</button>{% if cfg.reporte_pdf %}<a href="/reporte_pdf" class="btn btn-light me-2">📄 Descargar PDF</a>{% endif %}{% if cfg.total_whatsapp %}<a href="https://wa.me/{{cfg.numero_whatsapp}}?text={{msg_enc}}" target="_blank" class="btn-whats p-2">📲 Mandar total a mi WhatsApp</a>{% endif %}</div><table class="table table-dark table-bordered mt-4"><tr><th>Hora</th><th>Cliente</th><th>Producto</th><th>Total</th></tr>{% for v in ventas_hoy %}<tr><td>{{v.fecha.strftime("%H:%M")}}</td><td>{{v.cliente}}</td><td>{{v.cantidad}}x {{v.producto_nombre}}</td><td>${{v.total}}</td></tr>{% endfor %}</table></div></div>', ventas_hoy=ventas_hoy, total_hoy=total_hoy, cfg=cfg, msg_enc=msg_enc)
...
def reporte_pdf():
    ...
    hoy=datetime.now().replace(hour=0,minute=0,second=0,microsecond=0)
    ventas=Venta.query.filter(Venta.fecha>=hoy).all(); total=sum([v.total for v in ventas])
...
def admin_config():
    ...
    <p style="color:#aaa;font-size:12px">Activa o desactiva funciones - marca la palomita</p>
    <form method="POST" class="mt-4">
    <label style="display:flex;align-items:center;border:2px solid #333;border-radius:12px;padding:15px;margin-bottom:15px;cursor:pointer;background:#0a0a0a">
    <input type="checkbox" name="whatsapp_btn" style="width:28px;height:28px;accent-color:#ff4d8a" {{'checked' if cfg.whatsapp_btn}}> 
    <span style="margin-left:15px"><b>📲 1. Botón WhatsApp Directo</b><br><small style="color:#aaa">Mandar ticket por WhatsApp al cliente</small></span>
    </label>
    <label style="display:flex;align-items:center;border:2px solid #333;border-radius:12px;padding:15px;margin-bottom:15px;cursor:pointer;background:#0a0a0a">
    <input type="checkbox" name="tickets" style="width:28px;height:28px;accent-color:#ff4d8a" {{'checked' if cfg.tickets}}>
    <span style="margin-left:15px"><b>🎫 2. Tickets Imprimibles</b><br><small style="color:#aaa">Generar e imprimir tickets con logo Ruve</small></span>
    </label>
    <label style="display:flex;align-items:center;border:2px solid #333;border-radius:12px;padding:15px;margin-bottom:15px;cursor:pointer;background:#0a0a0a">
    <input type="checkbox" name="reporte_pdf" style="width:28px;height:28px;accent-color:#ff4d8a" {{'checked' if cfg.reporte_pdf}}>
    <span style="margin-left:15px"><b>📄 3. Reporte PDF</b><br><small style="color:#aaa">Descargar reporte del día en PDF</small></span>
    </label>
    <label style="display:flex;align-items:center;border:2px solid #333;border-radius:12px;padding:15px;margin-bottom:15px;cursor:pointer;background:#0a0a0a">
    <input type="checkbox" name="total_whatsapp" style="width:28px;height:28px;accent-color:#ff4d8a" {{'checked' if cfg.total_whatsapp}}>
    <span style="margin-left:15px"><b>💰 4. Enviar Total del Día a mi WhatsApp</b><br><small style="color:#aaa">Mandar total del día a tu número</small></span>
    </label>
    <div class="mb-4"><label style="color:#ff4d8a">Tu número de WhatsApp:</label>
    <input name="numero_whatsapp" class="form-control mt-2" value="{{cfg.numero_whatsapp}}" placeholder="529831234567"><small style="color:#666">Con código país sin + . Ej: México 52 + número</small></div>
    <button class="btn-rosa w-100" style="padding:14px;font-size:17px">💾 GUARDAR CONFIGURACIÓN</button>
    </form>
