...
app = Flask(__name__)
app.secret_key = 'ruve-stable-mesa4-fix'
...
def get_producto_imagen(p):
    ...
    try:
        return "/static/"+get_config().logo_path
    except:
...
<style>
:root{--rosa:#ff4d8a}body{background:#000;color:white}
...
.cat-btn{background:#222;color:white;padding:14px;border:2px solid #333;border-radius:8px;cursor:pointer;font-size:12px}
.cat-btn.active{background:var(--rosa)}
...
.label-rosa{color:var(--rosa);font-size:11px;margin-top:10px;display:block}
</style>
"""
...
def nav():
    ...
    if is_admin:
        links+='<a href="/productos" class="me-3">Productos</a><a href="/admin/config" class="me-3">Config</a><a href="/ventas" class="me-3">Ventas</a><a href="/admin/usuarios" class="me-3">Usuarios</a>'
...
def login():
    ...
    cfg=get_config()
    return render_template_string(STYLE_BASE+nav()+'<div style="display:flex;justify-content:center;margin-top:50px"><div class="card" style="width:350px;text-align:center"><h3 style="color:var(--rosa)">Ruve</h3><form method="POST"><input name="username" class="form-control mb-2" placeholder="Usuario" required><input name="password" type="password" class="form-control mb-2" placeholder="Contra" required><button class="btn-rosa w-100">Entrar</button></form></div></div>')

@app.route('/dashboard')
def dashboard():
    if 'user' not in session: return redirect('/')
    # QUERY SEGURA - no se cae si falta columna disponible
    all_prods = Producto.query.all()
    productos = [p for p in all_prods if getattr(p,'disponible',True)==True]
    prod_list=[]
    for p in productos:
        prod_list.append({'id':p.id,'nombre':p.nombre,'categoria':getattr(p,'categoria',''), 'img_url':get_producto_imagen(p), 'precio_mxn':format_mxn(p.precio or 0)})
...
<div class="pos-left">
<div style="padding:10px;border-bottom:2px solid var(--rosa)"><small style="color:var(--rosa)">TICKET MOSTRADOR - VENTA HOY {{total_hoy_mxn}}</small></div>
<div style="flex:1;overflow:auto;padding:10px;background:white;color:black">
...
<div style="padding:10px;border-top:2px solid var(--rosa)"><b>TOTAL ${{total}} MXN</b><br><button onclick="document.getElementById('modalEfe').classList.add('show')" style="background:#25D366;color:white;width:100%;padding:10px;border-radius:8px;margin-top:8px">EFECTIVO</button><br><a href="/pos/pagar/tarjeta" style="background:#3f51b5;color:white;display:block;text-align:center;padding:10px;border-radius:8px;margin-top:5px;text-decoration:none">Tarjeta</a><a href="/pos/clear" style="background:#333;color:white;display:block;text-align:center;padding:8px;border-radius:8px;margin-top:5px;text-decoration:none">Limpiar</a></div>
</div>
<div class="pos-center"><button class="cat-btn active">Todos</button></div>
<div class="pos-right"><div class="prod-grid">{% for p in productos %}<div class="prod-card" onclick="location='/pos/add/{{p.id}}'"><img src="{{p.img_url}}"><h6 style="color:var(--rosa)">{{p.nombre}}</h6><small style="color:var(--rosa)">{{p.precio_mxn}}</small></div>{% endfor %}</div></div>
</div>
<div id="modalEfe" class="modal-efectivo"><div class="modal-caja"><h5>Efectivo</h5><p>Total ${{total}} MXN</p><input id="entregado" type="number" class="form-control" placeholder="500" style="background:#000!important;color:white!important"><div style="margin-top:10px"><b id="cambio" style="color:#25D366;font-size:20px">$0</b></div><button onclick="var e=parseFloat(document.getElementById('entregado').value)||0; if(e<{{total_num}}) alert('menor'); else location.href='/pos/pagar/efectivo?entregado='+e;" style="background:#25D366;color:white;width:100%;padding:10px;border-radius:8px;margin-top:10px">Cobrar</button><button onclick="document.getElementById('modalEfe').classList.remove('show')" style="background:#333;color:white;width:100%;padding:8px;border-radius:8px;margin-top:5px">Cancelar</button></div></div>
""", productos=prod_list, carrito=[{'nombre':x['nombre'],'cant':x['cant'],'total_mxn':format_mxn(x['precio']*x['cant'])} for x in carrito], total="{:,.2f}".format(total), total_num=total, total_hoy_mxn=format_mxn(total_hoy), mesas_por_cobrar=[{'id':m.id,'nombre':m.nombre,'total_mxn':format_mxn(m.total or 0)} for m in mesas_por_cobrar])
...
def mesa_detalle(id):
    try:
        ...
        prod_list=[]
        for p in productos:
            prod_list.append({'id':p.id,'nombre':p.nombre,'precio_mxn':format_mxn(p.precio or 0),'img_url':get_producto_imagen(p)})
        carrito=session.get('mesa_carrito_'+str(id),[])
        if not isinstance(carrito, list):
            carrito=[]
            session['mesa_carrito_'+str(id)]=[]
...
""", mesa={'id':mesa.id,'nombre':mesa.nombre,'total_mxn':format_mxn(mesa.total or 0),'estado':mesa.estado}, productos=prod_list, carrito=[{'nombre':x.get('nombre',''),'cant':x.get('cant',0),'comentario':x.get('comentario',''),'total_mxn':format_mxn(float(x.get('precio',0))*int(x.get('cant',0)))} for x in carrito], comandas=comandas, total_final_mxn=format_mxn((mesa.total or 0)+total_nuevo), puede_cobrar=puede_cobrar)
        ...
        return "<h3>Error en mesa "+str(id)+"</h3><pre>"+traceback.format_exc()+"</pre><a href='/mesas'>Volver</a>"
...
def mesa_enviar(mesa_id):
    ...
    for it in carrito:
        ...
        mesa.total=(mesa.total or 0)+float(it.get('precio',0))*int(it.get('cant',0))
        mesa.estado='ocupada'
        db.session.add(com)
    db.session.commit()
    session[key]=[]
...
<table class="table table-sm"><tr><th>Cant</th><th>Producto</th></tr>
{% for c in comandas %}<tr><td>{{c.cantidad}}</td><td>{{c.producto_nombre}} {% if c.comentario %}<small style="color:red">({{c.comentario}})</small>{% endif %}</td></tr>{% endfor %}
</table>
<b>TOTAL {{total_mxn}}</b>
</div>
