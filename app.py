...
app = Flask(__name__)
app.secret_key = 'ruve-fix-502-profesional'
...
with app.app_context():
    try:
        ...
        if Mesa.query.count()==0:
            for i in range(1,12): db.session.add(Mesa(nombre=f"Mesa {i}"))
            db.session.commit()
...
.btn-rosa{background:#ff4d8a;color:white;border:none;padding:10px 18px;border-radius:8px;font-weight:600}
...
.navbar a{color:#fff;text-decoration:none;font-size:12px;font-weight:600;margin-right:18px;letter-spacing:0.3px}
...
.cat-btn{background:#1e1e1e;color:#fff;border:1.5px solid #2a2a2a;border-radius:8px;padding:12px;font-size:11px;font-weight:700;cursor:pointer}
.cat-btn.active{background:#ff4d8a;border-color:#ff4d8a}
.prod-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:9px}
.prod-card{background:#1a1a1a;border:1.5px solid #252525;border-radius:10px;padding:10px;text-align:center;cursor:pointer}
...
.ticket-header{background:#0f0f0f;padding:12px;border-bottom:1.5px solid #ff4d8a;display:flex;justify-content:space-between}
...
.btn-cash{background:#1f9d5a;color:white;font-weight:700;padding:11px;border:none;border-radius:8px;width:32%;font-size:11px}
.btn-pay{background:#2d4bcc;color:white;font-weight:700;padding:11px;border:none;border-radius:8px;width:32%;font-size:11px}
.btn-trans{background:#0e8a9a;color:white;font-weight:700;padding:11px;border:none;border-radius:8px;width:32%;font-size:11px}
.btn-clear{background:#2a2a2a;color:#ccc;border:1px solid #333;width:100%;padding:9px;border-radius:8px;font-size:11px;font-weight:600;margin-top:8px}
.dropdown{position:relative;display:inline-block}
.dropbtn{background:#111;color:#ff4d8a;border:1.5px solid #ff4d8a;padding:7px 14px;border-radius:8px;font-weight:700;cursor:pointer;font-size:11px}
.dropdown-content{display:none;position:absolute;right:0;background:#111;border:1.5px solid #ff4d8a;border-radius:10px;min-width:230px;z-index:9999}
.dropdown-content a{display:block;padding:11px 16px;color:#fff;text-decoration:none;font-size:12px;border-bottom:1px solid #1e1e1e}
.dropdown-content a:hover{background:#1a1a1a;color:#ff4d8a}
.dropdown-content.show{display:block}
.input-rosa{border:none!important;border-bottom:1.5px solid #ff4d8a!important;background:#111!important;color:#fff!important;border-radius:0!important;padding:10px 0!important;width:100%}
...
.select-rosa{background:#111!important;color:#fff!important;border:1.5px solid #ff4d8a!important;border-radius:8px!important;padding:8px!important}
.header-rosa{background:#000;border-bottom:2px solid #ff4d8a;color:#ff4d8a;padding:14px 20px;display:flex;align-items:center;font-weight:700;font-size:13px;letter-spacing:0.8px}
.card-negra{background:#111;border:1.5px solid #ff4d8a;border-radius:12px;padding:20px;margin:12px}
</style>
"""
...
def nav():
    ...
    if not is_admin and rol=='cocina':
        return f'<nav class="navbar"><div class="d-flex align-items-center"><img src="{logo_url}" style="width:34px;height:34px;border-radius:50%;background:white;padding:2px"><span style="margin-left:10px;font-weight:700;font-size:12px">COCINA - {nombre}</span></div><div><a href="/cocina">COCINA</a><a href="/logout">SALIR</a></div></nav>'
    links=""
    ...
    if is_admin:
        ...
        '''
    return f'<nav class="navbar"><div class="d-flex align-items-center"><img src="{logo_url}" style="width:34px;height:34px;border-radius:50%;background:white;padding:2px;object-fit:cover"><span style="margin-left:10px;font-weight:700;font-size:11px;color:#ff4d8a">RUVE - {nombre.upper()}</span></div><div>{links}{admin_drop}<a href="/logout">SALIR</a><span id="relojPC" style="margin-left:12px;color:#ff4d8a;font-size:10px;font-weight:700"></span></div></nav><script>function actualizarReloj(){{let a=new Date();document.getElementById("relojPC").textContent=a.toLocaleString("es-MX",{{hour12:false,timeZone:"America/Cancun"}})}};setInterval(actualizarReloj,1000);actualizarReloj();</script>'
...
<div class="pos-container">
    <div class="pos-left">
        <div class="ticket-header"><span style="font-size:10px;font-weight:700;color:#ff4d8a">TICKET MOSTRADOR</span><span id="fechaPOS" style="font-size:9px;color:#888"></span></div>
        <div class="ticket-body">
            {% for item in carrito %}<div style="display:flex;justify-content:space-between;border-bottom:1px dashed #ccc;padding:5px 0;font-size:11px"><span>{{item.nombre}} x{{item.cant}}</span><span>{{item.total_mxn}}</span></div>{% endfor %}
            {% if not carrito %}<p style="color:#888;text-align:center;margin-top:30px;font-size:11px">SELECCIONA UN PRODUCTO</p>{% endif %}
            {% if mesas_ocupadas %}<hr><div style="background:#fff3cd;color:#111;padding:6px;border-radius:6px;font-size:10px;font-weight:600">MESAS POR COBRAR: {% for m in mesas_ocupadas %}<div style="display:flex;justify-content:space-between"><span>{{m.nombre}} - {{m.total_mxn}}</span><a href="/mesa/{{m.id}}/cobrar" style="background:#1f9d5a;color:white;padding:2px 6px;border-radius:4px">COBRAR</a></div>{% endfor %}</div>{% endif %}
        </div>
        <div class="ticket-footer">
            <div style="display:flex;justify-content:space-between;color:white;font-weight:700;font-size:15px"><span>TOTAL</span><span>${{total}} MXN</span></div>
            ...
            <form method="POST" action="/pos/clear"><button type="submit" class="btn-clear">LIMPIAR TICKET</button></form>
        </div>
    </div>
...
<form method="POST" class="d-flex gap-2 mt-3"><input name="nombre" class="form-control" placeholder="Ej: Mesa 13" required style="background:#000!important;border:1.5px solid #ff4d8a!important"><button class="btn-rosa">AGREGAR</button></form>
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:10px;margin-top:15px">
{% for m in mesas %}<div style="background:#111;border:1.5px solid #2a2a2a;border-radius:10px;padding:12px;text-align:center"><b style="font-size:11px">{{m.nombre}}</b><br><small style="color:#888">{{m.estado.upper()}} - {{m.total_mxn}}</small><br><a href="/admin/mesas/eliminar/{{m.id}}" onclick="return confirm('Eliminar {{m.nombre}}? Solo si esta libre')" style="color:#ff4d8a;font-size:10px;font-weight:700">ELIMINAR</a></div>{% endfor %}
</div></div></div>
...
def mesas_view():
    mesas=Mesa.query.all()
    return render_template_string(STYLE_BASE+nav()+"""<div class="container mt-3"><h6 style="color:#ff4d8a;font-weight:700">MESAS</h6><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:10px;margin-top:10px">{% for m in mesas %}<div style="background:{% if m.estado=='ocupada' %}#2a1018{% else %}#101a14{% endif %};border:1.5px solid {% if m.estado=='ocupada' %}#ff4d8a{% else %}#1f9d5a{% endif %};padding:14px;border-radius:10px;text-align:center;cursor:pointer" onclick="location='/mesa/{{m.id}}'"><b style="font-size:11px">{{m.nombre}}</b><br><small style="font-size:9px">{{m.estado.upper()}}</small><br><b style="font-size:11px;color:#ff4d8a">{{m.total_mxn}}</b></div>{% endfor %}</div></div>""", mesas=[{'id':m.id,'nombre':m.nombre,'estado':m.estado,'total_mxn':format_mxn(m.total or 0)} for m in mesas])
...
def mesa_detalle(id):
    ...
    total_final=(mesa.total or 0)+sum([x['precio']*x['cant'] for x in carrito])
    return render_template_string(STYLE_BASE+nav()+"""
<div style="display:flex;height:calc(100vh - 58px);gap:10px;padding:10px">
<div style="width:42%;background:#111;border:1.5px solid #00e5ff;border-radius:12px;padding:12px;display:flex;flex-direction:column">
<h6 style="color:#00e5ff;font-weight:700;font-size:12px">{{mesa.nombre}} - {{mesa.total_mxn}}</h6>
<div style="flex:1;overflow:auto;margin-top:8px">
{% for c in comandas %}<div style="background:white;color:#111;padding:6px;border-radius:6px;margin-bottom:6px;font-size:11px"><b>{{c.cantidad}}x {{c.producto_nombre}}</b> {% if c.comentario %}<span style="background:#c62828;color:white;padding:2px 5px;border-radius:4px;font-size:10px">💬 {{c.comentario}}</span>{% endif %}</div>{% endfor %}
<b style="color:#ffcc00;font-size:10px">NUEVO - COMENTARIO ANTES COCINA</b>
...
<div style="border-top:1.5px solid #ff4d8a;padding-top:10px"><div style="display:flex;justify-content:space-between;font-weight:700"><span>TOTAL</span><span style="color:#ff4d8a">{{total_final_mxn}}</span></div><a href="/mesa/{{mesa.id}}/enviar" style="background:#00e5ff;color:#000;padding:10px;display:block;text-align:center;border-radius:8px;margin-top:8px;font-weight:700;font-size:11px;text-decoration:none">ENVIAR A COCINA</a><a href="/mesa/{{mesa.id}}/cobrar" style="background:#1f9d5a;color:white;padding:10px;display:block;text-align:center;border-radius:8px;margin-top:8px;font-weight:700;font-size:11px;text-decoration:none">COBRAR MESA</a></div>
</div>
<div style="width:58%;display:grid;grid-template-columns:repeat(3,1fr);gap:8px;overflow:auto;align-content:start">
{% for p in productos %}<div style="background:#fff;color:#111;border-radius:8px;padding:8px;text-align:center;cursor:pointer" onclick="location='/mesa/{{mesa.id}}/add/{{p.id}}'"><img src="{{p.img_url}}" style="width:56px;height:56px;object-fit:cover;border-radius:6px"><br><small style="font-weight:700;font-size:10px">{{p.nombre}}</small><br><small style="color:#ff4d8a;font-weight:700;font-size:11px">{{p.precio_mxn}}</small></div>{% endfor %}
</div>
</div>
""", mesa={'id':mesa.id,'nombre':mesa.nombre,'total_mxn':format_mxn(mesa.total or 0)}, productos=[{'id':p.id,'nombre':p.nombre,'precio_mxn':format_mxn(p.precio),'img_url':p.img_url} for p in productos], carrito=[{'nombre':x['nombre'],'cant':x['cant'],'comentario':x.get('comentario',''),'total_mxn':format_mxn(x['precio']*x['cant'])} for x in carrito], comandas=comandas, total_final_mxn=format_mxn(total_final))
...
def mesa_cobrar_view(id):
    ...
    return render_template_string(STYLE_BASE+nav()+"""<div class="container mt-4" style="max-width:460px"><div class="card"><h6 style="color:#1f9d5a;font-weight:700">COBRAR {{mesa.nombre}} - {{total_mxn}}</h6><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-top:15px"><a href="/mesa/{{mesa.id}}/cobrar_final/efectivo" style="background:#1f9d5a;color:white;padding:18px;text-align:center;border-radius:10px;font-weight:700;text-decoration:none;font-size:11px">EFECTIVO</a><a href="/mesa/{{mesa.id}}/cobrar_final/tarjeta" style="background:#2d4bcc;color:white;padding:18px;text-align:center;border-radius:10px;font-weight:700;text-decoration:none;font-size:11px">TARJETA</a><a href="/mesa/{{mesa.id}}/cobrar_final/transferencia" style="background:#0e8a9a;color:white;padding:18px;text-align:center;border-radius:10px;font-weight:700;text-decoration:none;font-size:11px">TRANSFER</a></div></div></div>""", mesa=mesa, total_mxn=format_mxn(total))
...
def cocina_view():
    ...
    return render_template_string(STYLE_BASE+nav()+"""<div class="container-fluid mt-3"><h6 style="color:#ffcc00;font-weight:700">COCINA - {{grupos_list|length}} MESAS</h6><div class="row g-3 mt-2">{% for g in grupos_list %}<div class="col-md-4"><div class="card" style="border-color:#ff3d57;background:#1a1212"><h6 style="color:#00e5ff;font-weight:700;font-size:11px">{{g.mesa.nombre}} - {{g.mesa.total_mxn}} - POR {{g.comandas[0].mesero_nombre}}</h6>{% for c in g.comandas %}<div style="background:white;color:#111;padding:6px;border-radius:6px;margin-bottom:6px;font-size:11px"><b>{{c.cantidad}}x {{c.producto_nombre}}</b> {% if c.comentario %}<span style="background:#c62828;color:white;padding:2px 6px;border-radius:4px;font-size:10px">💬 {{c.comentario}}</span>{% endif %}</div>{% endfor %}<a href="/cocina/mesa_listo/{{g.mesa.id}}" style="background:#1f9d5a;color:white;padding:8px;text-align:center;border-radius:8px;display:block;font-weight:700;font-size:11px;text-decoration:none">MESA LISTA</a></div></div>{% endfor %}{% if not grupos_list %}<div class="col-12 text-center p-4"><h5 style="color:#1f9d5a;font-weight:700">SIN PEDIDOS</h5></div>{% endif %}</div></div><script>setTimeout(()=>location.reload(),15000)</script>""", grupos_list=[{'mesa':{'id':g['mesa'].id,'nombre':g['mesa'].nombre,'total_mxn':format_mxn(g['mesa'].total or 0)},'comandas':g['comandas']} for g in grupos_list])
...
def productos_list():
    ...
    for p in productos: p.img_url=get_producto_imagen(p)
    return render_template_string(STYLE_BASE+nav()+"""<div class="container mt-3"><div class="card"><div style="display:flex;justify-content:space-between"><h6 style="color:#ff4d8a;font-weight:700">PRODUCTOS ({{productos|length}})</h6><a href="/productos/nuevo" style="background:#ff4d8a;color:white;padding:7px 14px;border-radius:8px;text-decoration:none;font-weight:700;font-size:11px">NUEVO ARTICULO</a></div><table class="table mt-3" style="color:white;font-size:11px"><tr><th style="color:#ff4d8a">FOTO</th><th>NOMBRE</th><th>CATEGORIA</th><th>PRECIO</th><th>STOCK</th><th></th></tr>{% for p in productos %}<tr><td><img src="{{p.img_url}}" style="width:40px;height:40px;object-fit:cover;border-radius:6px;background:white"></td><td>{{p.nombre}}</td><td>{{p.categoria}}</td><td style="color:#ff4d8a;font-weight:700">{{p.precio_mxn}}</td><td>{{p.stock}}</td><td><a href="/productos/eliminar/{{p.id}}" style="color:#ff4d8a">ELIMINAR</a></td></tr>{% endfor %}</table></div></div>""", productos=[{'id':p.id,'nombre':p.nombre,'categoria':p.categoria,'stock':p.stock,'img_url':p.img_url,'precio_mxn':format_mxn(p.precio)} for p in productos])
...
<div class="header-rosa">CREAR ARTICULO <a href="/productos" style="margin-left:auto;background:#111;color:#ff4d8a;border:1.5px solid #ff4d8a;padding:6px 12px;border-radius:8px;text-decoration:none;font-size:10px">VOLVER</a></div>
<form method="POST" enctype="multipart/form-data">
<div class="card-negra">
<div class="row"><div class="col-md-6"><label class="label-rosa">Nombre *</label><input name="nombre" class="input-rosa" placeholder="Ej: Agua 500ml" style="font-size:15px;font-weight:600" required></div>
<div class="col-md-6"><label class="label-rosa">Categoria</label><div style="display:flex;gap:6px"><select name="categoria" id="catSelect" class="select-rosa" style="flex:1">{% for cat in categorias %}<option value="{{cat.nombre}}">{{cat.nombre}}</option>{% endfor %}</select><button type="button" onclick="nuevaCategoria()" style="background:#ff4d8a;color:white;border:none;padding:6px 12px;border-radius:8px;font-size:10px;font-weight:700">+ NUEVA</button></div></div></div>
<div class="row"><div class="col-md-4"><label class="label-rosa">Precio MXN *</label><input name="precio" type="number" step="0.01" class="input-rosa" placeholder="0.00" required></div><div class="col-md-4"><label class="label-rosa">Stock</label><input name="en_stock" type="number" class="input-rosa" placeholder="0"></div><div class="col-md-4"><label class="label-rosa">Foto</label><input name="imagen" type="file" class="form-control" accept="image/*" style="background:#111!important;color:white!important;border:1.5px solid #ff4d8a!important;font-size:11px"></div></div>
<div style="margin-top:12px"><label style="font-size:11px"><input type="checkbox" name="disponible" checked style="accent-color:#ff4d8a"> Disponible</label></div>
<button style="background:#ff4d8a;color:white;border:none;padding:10px 22px;border-radius:8px;font-weight:700;margin-top:18px;font-size:12px">GUARDAR ARTICULO</button>
</div>
</form>
</div>
<script>
...
def dueno_view():
    if not session.get('is_admin'): return redirect('/dashboard')
    return render_template_string(STYLE_BASE+nav()+"""
<div class="container mt-3"><div class="card"><h6 style="color:#ff4d8a;font-weight:700">REPORTE VENTAS VS GASTOS</h6>
<div style="display:flex;gap:10px;margin-top:12px"><select id="periodo" class="form-control" style="width:170px;background:#000!important;color:white!important;border:1.5px solid #ff4d8a!important;font-size:11px" onchange="cargarGrafica()"><option value="dia">DIA - 7 DIAS</option><option value="mes" selected>MES - 12 MESES</option><option value="ano">AÑO</option></select><button onclick="cargarGrafica()" style="background:#ff4d8a;color:white;border:none;padding:7px 14px;border-radius:8px;font-weight:700;font-size:10px">ACTUALIZAR</button></div>
...
<div class="row mt-3 g-2"><div class="col-md-4"><div class="card" style="padding:12px"><small style="color:#888;font-size:10px">VENTAS</small><h6 id="totalVentas" style="color:#1f9d5a;margin:0">$0.00 MXN</h6></div></div><div class="col-md-4"><div class="card" style="padding:12px"><small style="color:#888;font-size:10px">GASTOS</small><h6 id="totalGastos" style="color:#ff4d8a;margin:0">$0.00 MXN</h6></div></div><div class="col-md-4"><div class="card" style="padding:12px;background:#1a0a10"><small style="color:#888;font-size:10px">GANANCIA</small><h6 id="totalGanancia" style="color:#ff4d8a;margin:0">$0.00 MXN</h6></div></div></div>
<div class="card mt-3"><h6 style="font-size:10px;color:#ff4d8a">REGISTRAR GASTO</h6><form id="formGasto" class="d-flex gap-2 mt-2"><input id="concepto" class="form-control" placeholder="Concepto" required style="background:#000!important;border:1.5px solid #ff4d8a!important;font-size:11px"><input id="monto" type="number" step="0.01" class="form-control" placeholder="$ MXN" required style="background:#000!important;border:1.5px solid #ff4d8a!important;font-size:11px"><button style="background:#ff4d8a;color:white;border:none;padding:7px 14px;border-radius:8px;font-weight:700;font-size:10px">AGREGAR</button></form></div>
</div></div>
<script>
let chart;
...
def admin_config():
    ...
    return render_template_string(STYLE_BASE+nav()+"""<div class="container mt-4" style="max-width:500px"><div class="card"><h6 style="color:#ff4d8a;font-weight:700">CONFIGURACION</h6><div class="text-center"><img src="/static/{{cfg.logo_path}}" style="width:80px;height:80px;border-radius:50%;background:white;padding:3px;border:2px solid #ff4d8a;object-fit:cover"></div><form method="POST" enctype="multipart/form-data" class="mt-3"><label class="label-rosa">Logo</label><input name="logo" type="file" class="form-control mb-2" accept="image/*" style="background:#000!important;border:1.5px solid #ff4d8a!important;font-size:11px"><div class="form-check mt-2"><input type="checkbox" name="mod_pos_mesero" {{'checked' if cfg.mod_pos_mesero}} style="accent-color:#ff4d8a"><label style="font-size:11px"> Habilitar POS para mesero</label></div><button class="btn-rosa w-100 mt-3" style="font-size:11px">GUARDAR</button></form></div></div>""", cfg=cfg)
...
<div class="container mt-4" style="max-width:350px"><div class="card" style="background:white;color:#111;border:2px solid #ff4d8a"><div class="text-center"><img src="/static/{cfg.logo_path}" style="width:60px;height:60px;border-radius:50%;background:white;padding:2px;border:2px solid #ff4d8a;object-fit:cover"><h6 style="font-weight:700;margin-top:6px;font-size:13px">RUVE LECHON</h6><small style="font-size:9px">TICKET #{v.id}</small><br><small id="fechaPC" style="font-size:9px;color:#666"></small></div><hr style="margin:8px 0"><div style="font-size:11px"><b>Cliente:</b> {v.cliente}<br><b>Producto:</b> {v.producto_nombre}<br><b>Cant:</b> {v.cantidad}<br><b>Total:</b> <span style="color:#ff4d8a;font-weight:700">{format_mxn(v.total)}</span><br><b>Pago:</b> {v.metodo_pago.upper()}<br><b>Atendio:</b> {v.vendedor_nombre or v.vendedor}<br><small style="color:#888;font-size:9px">Usuario: {v.vendedor}</small></div><hr><button onclick="window.print()" class="btn-rosa w-100" style="font-size:11px">IMPRIMIR</button><a href="/dashboard" class="btn btn-dark w-100 mt-2" style="font-size:11px">VOLVER POS</a></div></div>
...
def ventas():
    ...
    return render_template_string(STYLE_BASE+nav()+"""<div class="container mt-3"><div class="card"><h6 style="color:#ff4d8a;font-weight:700">VENTAS - MXN</h6><table class="table table-dark table-sm" style="font-size:10px"><tr><th style="color:#ff4d8a">FECHA</th><th>CLIENTE</th><th>PRODUCTO</th><th>TOTAL</th><th>PAGO</th><th>VENDEDOR</th></tr>{% for v in vs %}<tr><td>{{v.fecha.strftime('%d/%m %H:%M')}}</td><td>{{v.cliente}}</td><td>{{v.producto_nombre}} x{{v.cantidad}}</td><td style="color:#ff4d8a;font-weight:700">{{v.total_mxn}}</td><td>{{v.metodo_pago.upper()}}</td><td>{{v.vendedor_nombre}}</td></tr>{% endfor %}</table></div></div>""", vs=[{'fecha':v.fecha,'cliente':v.cliente,'producto_nombre':v.producto_nombre,'cantidad':v.cantidad,'total_mxn':format_mxn(v.total),'metodo_pago':v.metodo_pago,'vendedor_nombre':v.vendedor_nombre or v.vendedor} for v in vs])
...
def admin_usuarios():
    ...
    return render_template_string(STYLE_BASE+nav()+"""
<div class="container mt-3"><div class="card"><h6 style="color:#ff4d8a;font-weight:700">USUARIOS</h6><p style="font-size:9px;color:#666">Nombre completo aparece solo en ticket</p>
<form method="POST" class="row g-2 mt-2">
<div class="col-md-3"><label class="label-rosa">Nombre completo *</label><input name="nombre_completo" class="form-control" placeholder="Ej: Juan Perez Lopez" required style="background:#000!important;border:1.5px solid #ff4d8a!important;font-size:11px"></div>
<div class="col-md-2"><label class="label-rosa">Usuario *</label><input name="username" class="form-control" placeholder="jperez" required style="background:#000!important;border:1.5px solid #ff4d8a!important;font-size:11px"></div>
<div class="col-md-2"><label class="label-rosa">Contraseña *</label><input name="password" type="password" class="form-control" required style="background:#000!important;border:1.5px solid #ff4d8a!important;font-size:11px"></div>
<div class="col-md-2"><label class="label-rosa">Rol</label><select name="rol" class="form-control" style="background:#000!important;border:1.5px solid #ff4d8a!important;font-size:11px"><option value="cajero">Cajero</option><option value="mesero">Mesero</option><option value="cocina">Cocina</option></select></div>
<div class="col-md-1"><label class="label-rosa">Admin</label><br><input type="checkbox" name="is_admin" style="accent-color:#ff4d8a"></div>
<div class="col-md-2"><label class="label-rosa"> </label><button class="btn-rosa w-100" style="font-size:10px">CREAR</button></div>
</form>
<table class="table table-dark mt-3" style="font-size:11px"><tr><th style="color:#ff4d8a">NOMBRE COMPLETO</th><th>USUARIO</th><th>ROL</th><th></th></tr>
