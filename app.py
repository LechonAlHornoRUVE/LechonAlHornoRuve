...
app = Flask(__name__)
app.secret_key = 'ruve-final-completo-fotos-dropdown-cocina-fix'
...
def get_producto_imagen(p):
    ...
    cfg=get_config()
    return f"/static/{cfg.logo_path}" if cfg.logo_path else "/static/logo.png"
...
.navbar{background:#000!important;border-bottom:2px solid #ff4d8a;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap}
...
def nav():
    ...
    if not is_admin and rol=='cocina':
        return f'<nav class="navbar p-3 no-print"><div class="d-flex align-items-center"><img src="{logo_url}" style="width:40px;height:40px;border-radius:50%;background:white;padding:3px;object-fit:cover"><h6 class="m-0 ms-2" style="color:#ffcc00">Cocina {user}</h6></div><div><a href="/cocina" class="me-3" style="color:#ffcc00">🔥 Cocina</a><a href="/logout">Salir</a></div></nav>'
    links=""
...
def dashboard():
    ...
    for p in productos: p.img_url=get_producto_imagen(p); p.cat=get_categoria(p.nombre)
...
<div style="width:40%;background:#0f0f0f;border:2px solid #ff4d8a;border-radius:15px;display:flex;flex-direction:column;margin:10px">
<div style="padding:10px;background:white;color:black;flex:1;overflow:auto">
...
<div style="padding:10px;border-top:2px solid #ff4d8a"><b>Total ${{total}}</b><div style="display:flex;gap:5px;margin-top:8px"><button onclick="pagar('efectivo')" style="background:#25D366;color:white;border:none;padding:8px;border-radius:6px;width:33%">Efectivo</button><button onclick="pagar('tarjeta')" style="background:#3f51b5;color:white;border:none;padding:8px;border-radius:6px;width:33%">Tarjeta</button><button onclick="pagar('transferencia')" style="background:#0097a7;color:white;border:none;padding:8px;border-radius:6px;width:33%">Transfer</button></div></div>
</div>
<div style="width:60%;padding:10px;overflow:auto"><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px">
{% for p in productos %}<div style="background:white;color:#333;border-radius:8px;padding:8px;text-align:center;cursor:pointer" onclick="location='/pos/add/{{p.id}}'"><img src="{{p.img_url}}" style="width:70px;height:70px;object-fit:cover;border-radius:8px;background:#eee"><br><b style="font-size:11px">{{p.nombre}}</b><br><small>${{p.precio}}</small><br><small style="color:#888">{{p.categoria}}</small></div>{% endfor %}
</div></div>
</div>
<script>
function pagar(tipo){let cliente='Mostrador'; fetch('/pos/pagar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cliente:cliente,tipo:tipo})}).then(r=>r.json()).then(d=>{if(d.ok)location='/ticket/'+d.ticket_id; else alert(d.error)})}
</script>
""", productos=productos, carrito=carrito, total=total, mesas_ocupadas=mesas_ocupadas)
...
def mesa_detalle(id):
    ...
    return render_template_string(STYLE_BASE+nav()+"""
<div style="display:flex;height:calc(100vh - 70px);gap:10px;padding:10px">
...
{% for c in comandas %}<div style="background:white;color:black;padding:6px;border-radius:6px;margin-bottom:5px;font-size:12px"><b>{{c.cantidad}}x {{c.producto_nombre}}</b> {% if c.comentario %}<span style="background:#c62828;color:white;padding:2px 4px;border-radius:4px">💬 {{c.comentario}}</span>{% endif %}<br>
<form action="/mesa/{{mesa.id}}/comanda/coment/{{c.id}}" method="POST" style="display:flex;gap:3px;margin-top:3px"><input name="comentario" value="{{c.comentario}}" style="font-size:11px" class="form-control" placeholder="Editar comentario"><button style="font-size:10px">Guardar</button></form>
</div>{% endfor %}
<hr>
<b style="color:#ffcc00;font-size:12px">Nuevo ticket - comentario ANTES</b>
{% for it in carrito %}<div style="background:#fffde7;color:black;padding:5px;border-radius:4px;margin-top:5px;font-size:12px">{{it.nombre}} x{{it.cant}}<form action="/mesa/{{mesa.id}}/carrito/coment/{{loop.index0}}" method="POST" style="display:flex;gap:3px"><input name="comentario" value="{{it.comentario}}" class="form-control" style="font-size:11px" placeholder="💬 Comentario antes"><button>💾</button></form></div>{% endfor %}
<div style="margin-top:10px"><b>Total Final ${{(mesa.total or 0)+total_nuevo}}</b><br>
<a href="/mesa/{{mesa.id}}/enviar" style="background:#00e5ff;color:black;padding:8px;display:block;text-align:center;border-radius:6px;margin-top:5px">MANDAR A COCINA</a>
<a href="/mesa/{{mesa.id}}/cobrar" style="background:#25D366;color:white;padding:8px;display:block;text-align:center;border-radius:6px;margin-top:5px">💰 COBRAR MESA</a></div>
</div>
...
def mesa_add(mesa_id, prod_id):
    mesa=Mesa.query.get(mesa_id); prod=Producto.query.get(prod_id); carrito=session.get(f'mesa_carrito_{mesa_id}',[])
    for it in carrito:
...
def comanda_coment(mesa_id,com_id):
    c=Comanda.query.get(com_id); c.comentario=request.form.get('comentario','')[:200]; c.editado=True; db.session.commit(); return redirect(f'/mesa/{mesa_id}')
@app.route('/mesa/<int:id>/cobrar')
def mesa_cobrar_view(id):
    mesa=Mesa.query.get(id); total=mesa.total or 0
    return render_template_string(STYLE_BASE+nav()+"""
<div class="container mt-4" style="max-width:500px"><div class="card" style="border-color:#25D366"><h4>💰 Cobrar {{mesa.nombre}} - ${{total}}</h4>
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-top:15px">
<a href="/mesa/{{mesa.id}}/cobrar_final/efectivo" style="background:#25D366;color:white;padding:20px;text-align:center;border-radius:8px">💵<br>Efectivo</a>
<a href="/mesa/{{mesa.id}}/cobrar_final/tarjeta" style="background:#3f51b5;color:white;padding:20px;text-align:center;border-radius:8px">💳<br>Tarjeta</a>
<a href="/mesa/{{mesa.id}}/cobrar_final/transferencia" style="background:#0097a7;color:white;padding:20px;text-align:center;border-radius:8px">🏦<br>Transfer</a>
</div></div></div>
""", mesa=mesa, total=total)
...
# --- COCINA (AQUI ESTABA EL ERROR, YA RESTAURADO) ---
@app.route('/cocina')
def cocina_view():
    if 'user' not in session: return redirect('/')
    ...
    for mesa_id, lista in grupos.items():
        ...
        grupos_list.append({'mesa':mesa,'comandas':lista})
    return render_template_string(STYLE_BASE+nav()+"""
<div class="container-fluid mt-3"><h3 style="color:#ffcc00">🔥 Cocina - {{grupos_list|length}} mesas</h3>
<div class="row g-3 mt-2">
{% for g in grupos_list %}<div class="col-md-4"><div class="card" style="border-color:#ff4d3a;background:#1a1a0a"><h5 style="color:#00e5ff">🪑 {{g.mesa.nombre}}</h5>
{% for c in g.comandas %}<div style="background:white;color:black;padding:6px;border-radius:6px;margin-bottom:5px"><b>{{c.cantidad}}x {{c.producto_nombre}}</b><br>{% if c.comentario %}<span style="background:#c62828;color:white;padding:2px 6px;border-radius:4px;font-size:11px">💬 {{c.comentario}}</span>{% endif %}<br><small>{{c.mesero}}</small></div>{% endfor %}
<a href="/cocina/mesa_listo/{{g.mesa.id}}" class="btn-rosa w-100 mt-2" style="background:#25D366">✅ MESA LISTA</a>
</div></div>{% endfor %}
...
def productos_list():
    ...
    for p in productos: p.img_url=get_producto_imagen(p)
    return render_template_string(STYLE_BASE+nav()+"""
<div class="container mt-3"><div style="background:white;color:#333;border-radius:4px;padding:15px">
<div style="display:flex;justify-content:space-between"><h4>📦 Productos ({{productos|length}})</h4><a href="/productos/nuevo" style="background:#4caf50;color:white;padding:8px 16px;border-radius:4px;text-decoration:none">+ Crear artículo</a></div>
<table class="table mt-3"><tr><th>Foto</th><th>Nombre</th><th>Categoría</th><th>Precio</th><th>Stock</th><th></th></tr>
{% for p in productos %}<tr><td><img src="{{p.img_url}}" style="width:45px;height:45px;object-fit:cover;border-radius:6px"></td><td>{{p.nombre}}<br><small style="color:#888">{{p.descripcion[:30]}}</small></td><td>{{p.categoria}}</td><td>${{p.precio}}</td><td>{{p.stock}}</td><td><a href="/productos/nuevo?edit={{p.id}}" style="color:#4caf50">Editar</a> | <a href="/productos/eliminar/{{p.id}}" style="color:red">Borrar</a></td></tr>{% endfor %}
</table></div></div>
""", productos=productos)
...
<div class="row"><div class="col-md-8"><label class="label-small">Nombre</label><input name="nombre" class="input-line" placeholder="Ej: Agua, botella 0.5L" style="font-size:22px;font-weight:bold"></div>
<div class="col-md-4"><label class="label-small">Categoría</label><select name="categoria" class="input-line"><option>Sin categoría</option><option>Lechón</option><option>Tortas</option><option>Órdenes</option><option>Bebidas</option><option>Extras</option></select></div></div>
...
<div style="margin-top:15px"><label class="label-small">Foto (visible para cajero, mesero, cocina)</label><input name="imagen" type="file" class="form-control" accept="image/*"></div>
</div>
<div class="card-blanca"><h5>Inventario</h5>
<div style="display:flex;justify-content:space-between;margin-top:15px"><label>Artículo compuesto</label><label class="switch"><input type="checkbox" name="articulo_compuesto"><span class="slider"></span></label></div>
<div style="display:flex;justify-content:space-between;margin-top:10px"><label>Seguir el Inventario</label><label class="switch"><input type="checkbox" name="seguir_inventario" checked><span class="slider"></span></label></div>
<div class="row" style="margin-top:10px"><div class="col-md-6"><label class="label-small">En stock</label><input name="en_stock" type="number" class="input-line" placeholder="0"></div><div class="col-md-6"><label class="label-small">Inventario bajo</label><input name="inventario_bajo" type="number" class="input-line"><small style="font-size:10px;color:#888">Cantidad mínima para notificación</small></div></div>
<div style="margin-top:10px"><label class="label-small">Stock óptimo</label><input name="stock_optimo" type="number" class="input-line" style="width:50%"></div>
<div class="row" style="margin-top:15px"><div class="col-md-6"><label class="label-small">Proveedor principal</label><input name="proveedor" class="input-line"></div><div class="col-md-6"><label class="label-small">Costo de compra por defecto</label><input name="costo_compra" type="number" step="0.01" class="input-line"></div></div>
...
<form method="POST" enctype="multipart/form-data" class="mt-3"><label>Cambiar logo (se refleja en login y navbar)</label><input name="logo" type="file" class="form-control mb-3" accept="image/*"><button class="btn-rosa w-100">Guardar Logo</button></form></div></div>
""", cfg=cfg)

@app.route('/ticket/<int:id>')
def ticket(id):
    v=Venta.query.get(id); return render_template_string(STYLE_BASE+f'<div class="container mt-5" style="max-width:350px"><div class="card" style="background:white;color:black"><h5>Ticket #{v.id}</h5><p>{v.producto_nombre}<br>Total ${v.total}<br>Pago {v.metodo_pago}</p><button onclick="window.print()" class="btn-rosa">Imprimir</button></div></div>')
...
<table class="table table-dark mt-3"><tr><th>Usuario</th><th>Rol</th></tr>{% for u in usuarios %}<tr><td>{{u.username}}</td><td>{{u.rol}}</td></tr>{% endfor %}</table></div></div>""", usuarios=User.query.all())
