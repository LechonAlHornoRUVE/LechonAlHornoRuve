...
app = Flask(__name__)
app.secret_key = 'ruve-fix-final-imagen-y-cobro-caja'
...
try:
    ...
    if os.environ.get('CLOUDINARY_CLOUD_NAME'): CLOUDINARY_ENABLED=True
except: pass
...
def save_upload(file):
    ...
    try:
        ...
        fn=f"{datetime.now().strftime('%Y%m%d%H%M%S%f')}_{secure_filename(file.filename)}"
...
def get_producto_imagen(p):
    try:
        ...
        if p and p.imagen and 'uploads' in p.imagen:
            ...
            if os.path.exists(full): return f"/static/{p.imagen}"
    except: pass
    cfg=get_config()
...
.crear-wrapper{background:#000;min-height:100vh}.crear-header{background:var(--rosa);color:white;padding:12px 20px;display:flex;justify-content:space-between;align-items:center;font-weight:bold}
...
def nav():
    ...
    if not is_admin and rol=='cocina':
        return f'<nav class="navbar"><div class="d-flex align-items-center"><img src="{logo_url}" style="width:40px;height:40px;border-radius:50%;background:white;padding:3px"><h6 class="m-0 ms-2" style="color:#ffcc00">Cocina {nombre}</h6></div><div><a href="/cocina" class="me-3">🔥 Cocina</a><a href="/logout">Salir</a></div></nav>'
    links=""
    ...
    admin_drop=""
    if is_admin:
        admin_drop='<div class="dropdown"><button onclick="toggleDropdown()" class="dropbtn">⚙️ Herramientas admin ▾</button><div id="adminDropdown" class="dropdown-content"><a href="/productos">📦 Productos</a><a href="/productos/nuevo">➕ Crear artículo</a><a href="/admin/categorias">🏷️ Categorías</a><a href="/admin/mesas">🪑 Mesas</a><a href="/ventas">🧾 Ventas</a><a href="/dueno">💰 Dueño + Gráfica</a><a href="/admin/config">⚙️ Config + Logo</a><a href="/admin/usuarios">👥 Usuarios</a></div></div><script>function toggleDropdown(){document.getElementById("adminDropdown").classList.toggle("show");}</script>'
    return f'<nav class="navbar"><div class="d-flex align-items-center"><img src="{logo_url}" style="width:40px;height:40px;border-radius:50%;background:white;padding:3px"><h6 class="m-0 ms-2" style="color:var(--rosa)">Ruve {nombre}</h6><span id="relojPC" style="margin-left:12px;color:var(--rosa);font-size:10px;font-weight:700"></span></div><div>{links}{admin_drop}<a href="/logout" class="ms-3">Salir</a></div></nav><script>setInterval(()=>{let el=document.getElementById("relojPC"); if(el) el.textContent=new Date().toLocaleString("es-MX",{timeZone:"America/Cancun"})},1000)</script>'
...
def login():
    ...
    return render_template_string(STYLE_BASE+f'<div style="min-height:100vh;display:flex;justify-content:center;align-items:center;background:#000;padding:20px"><div class="card" style="width:100%;max-width:380px;text-align:center;padding:30px 25px"><img src="{logo_url}" style="width:130px;height:130px;border-radius:50%;background:white;padding:5px;border:3px solid var(--rosa)"><h3 style="color:var(--rosa);font-weight:bold;margin:15px 0">Ruve</h3>{"<div style=background:#2a1018;border:1px solid var(--rosa);color:var(--rosa);padding:8px;border-radius:8px;font-size:12px;margin-bottom:10px>"+error+"</div>" if error else ""}<form method="POST"><input name="username" class="form-control mb-3" placeholder="Usuario" required style="background:white!important;color:#333!important;padding:12px"><input name="password" type="password" class="form-control mb-3" placeholder="Contraseña" required style="background:white!important;color:#333!important;padding:12px"><button class="btn-rosa w-100">Entrar</button></form><div style="margin-top:15px"><a href="/restablecer" style="color:var(--rosa);font-size:12px">¿Olvidaste contraseña?</a></div></div></div>')
...
def restablecer():
    ...
    if request.method=='POST':
        ...
        elif nueva!=confirmar: error="No coinciden"
        elif len(nueva)<4: error="Mínimo 4"
        else: u.password=generate_password_hash(nueva); db.session.commit(); msg="Restablecida"
    logo_url=get_producto_imagen(cfg)
    return render_template_string(STYLE_BASE+f'<div style="min-height:100vh;display:flex;justify-content:center;align-items:center"><div class="card" style="max-width:400px;text-align:center"><img src="{logo_url}" style="width:90px;height:90px;border-radius:50%;background:white;padding:4px;border:2px solid var(--rosa)"><h5 style="color:var(--rosa)">Restablecer</h5>{"<div style=background:#101a10;border:1px solid #25D366;color:#25D366;padding:8px;border-radius:8px>"+msg+"</div>" if msg else ""}{"<div style=background:#2a1018;border:1px solid var(--rosa);color:var(--rosa);padding:8px;border-radius:8px>"+error+"</div>" if error else ""}<form method="POST" class="text-start mt-3"><label class="label-rosa">Usuario</label><input name="username" class="form-control mb-2" required><label class="label-rosa">Nueva</label><input name="nueva" type="password" class="form-control mb-2" required><label class="label-rosa">Confirmar</label><input name="confirmar" type="password" class="form-control mb-3" required><button class="btn-rosa w-100">Restablecer</button></form><a href="/" style="color:#888;font-size:12px">← Volver</a></div></div>')

@app.route('/dashboard')
def dashboard():
    if 'user' not in session: return redirect('/')
    cfg=get_config()
    if not session.get('is_admin') and session.get('rol')=='mesero' and not cfg.mod_pos_mesero: return redirect('/mesas')
    if not session.get('is_admin') and session.get('rol')=='cocina': return redirect('/cocina')
    productos=Producto.query.filter_by(disponible=True).all()
    productos_list=[{'id':p.id,'nombre':p.nombre,'categoria':p.categoria,'stock':p.stock,'img_url':get_producto_imagen(p),'precio_mxn':format_mxn(p.precio or 0)} for p in productos]
...
<div class="pos-container">
    <div class="pos-left">
        ...
        <div class="ticket-body">
            ...
            {% if mesas_por_cobrar %}
            <div style="background:#ffeb3b;color:black;padding:8px;border-radius:8px;margin-top:10px;border:2px solid #ff9800;animation: parpadeo 1s infinite alternate"><b style="font-size:11px">🔔 SOLICITUDES DE COBRO (MESERO A CAJA):</b>
...
function actualizarTotalHoy(){fetch('/api/total_hoy').then(r=>r.json()).then(d=>{document.getElementById('totalHoy').textContent=d.total_mxn;});}setInterval(actualizarTotalHoy,5000);
function abrirEfectivo(){if(totalNum<=0){alert('Ticket vacío');return;}document.getElementById('modalTotal').textContent='$'+totalNum.toFixed(2)+' MXN';document.getElementById('cantidadEntregada').value=totalNum.toFixed(2);calcularCambio();document.getElementById('modalEfectivo').classList.add('show');}
function cerrarEfectivo(){document.getElementById('modalEfectivo').classList.remove('show');}
function calcularCambio(){let e=parseFloat(document.getElementById('cantidadEntregada').value)||0;document.getElementById('modalCambio').textContent='$'+(e-totalNum).toFixed(2)+' MXN';}
function confirmarEfectivo(){let e=parseFloat(document.getElementById('cantidadEntregada').value)||0;if(e<totalNum){alert('Menor al total');return;}location.href='/pos/pagar/efectivo?entregado='+e;}
</script>
...
def pos_pagar_direct(metodo):
    ...
    entregado=request.args.get('entregado', type=float); last_id=None; total_ticket=0
    ...
    if last_id:
        if metodo=='efectivo' and entregado is not None: return redirect(f'/ticket/{last_id}?entregado={entregado}&cambio={entregado-total_ticket}')
...
def admin_categorias_editar(id):
    ...
    if request.method=='POST':
        ...
        if nuevo:
            ...
            db.session.commit()
        return redirect('/admin/categorias')
    return render_template_string(STYLE_BASE+nav()+f'<div class="container mt-3"><div class="card" style="max-width:400px;margin:0 auto"><h5>Editar Categoría</h5><form method="POST"><input name="nombre" class="form-control mb-3" value="{cat.nombre}" required><button class="btn-rosa w-100">Guardar</button></form></div></div>')
...
def mesa_detalle(id):
    cfg=get_config(); is_admin=session.get('is_admin'); rol=session.get('rol'); puede_cobrar=is_admin or rol=='cajero' or cfg.mod_mesero_cobrar
...
def mesa_cobrar_view(id):
    cfg=get_config(); is_admin=session.get('is_admin'); rol=session.get('rol'); mesa=Mesa.query.get(id)
...
<p style="color:#888;font-size:12px">Esta es la pantalla de COBRO separada de POS. Aquí cobra caja, no el mesero si no tiene permiso.</p>
...
""", mesa={'id':mesa.id,'nombre':mesa.nombre}, total_mxn=format_mxn(mesa.total or 0), comandas=[{'cantidad':c.cantidad,'producto_nombre':c.producto_nombre,'comentario':c.comentario,'total_mxn':format_mxn((Producto.query.filter_by(nombre=c.producto_nombre).first().precio if Producto.query.filter_by(nombre=c.producto_nombre).first() else 0)*(c.cantidad or 1))} for c in comandas])
...
def productos_nuevo():
    ...
    return render_template_string(STYLE_BASE+nav()+"""<div class="crear-wrapper"><div class="crear-header"><span>☰ Crear artículo</span><a href="/productos" style="background:#000;color:white;padding:6px 14px;border-radius:6px;text-decoration:none;border:1px solid white">← Volver</a></div><form method="POST" enctype="multipart/form-data"><div class="crear-card"><div class="row"><div class="col-md-6"><label class="label-rosa">Nombre *</label><input name="nombre" class="crear-input" required></div><div class="col-md-6"><label class="label-rosa">Categoría</label><select name="categoria" class="form-control" style="background:#111!important;color:white!important">{% for cat in categorias %}<option>{{cat.nombre}}</option>{% endfor %}</select></div></div><div class="row" style="margin-top:20px"><div class="col-md-6"><label class="label-rosa">Precio</label><input name="precio" type="number" step="0.01" class="crear-input" required></div><div class="col-md-6"><label class="label-rosa">Stock</label><input name="stock" type="number" class="crear-input" value="0"></div></div><div style="margin-top:20px"><label class="label-rosa">Foto {% if cloudinary_enabled %}(Permanente ☁️){% endif %}</label><input name="imagen" type="file" class="form-control" accept="image/*"></div><button style="background:var(--rosa);color:white;border:none;padding:12px 30px;border-radius:6px;font-weight:bold;margin-top:20px">💾 GUARDAR</button></div></form></div>""", categorias=categorias, cloudinary_enabled=CLOUDINARY_ENABLED)
...
def productos_editar(id):
    ...
    return render_template_string(STYLE_BASE+nav()+"""<div class="crear-wrapper"><div class="crear-header"><span>✏️ Editar {{p.nombre}}</span><a href="/productos" style="background:#000;color:white;padding:6px 14px;border-radius:6px;text-decoration:none;border:1px solid white">← Volver</a></div><form method="POST" enctype="multipart/form-data"><div class="crear-card"><label class="label-rosa">Nombre *</label><input name="nombre" class="crear-input" value="{{p.nombre}}" required><div class="row" style="margin-top:15px"><div class="col-md-6"><label class="label-rosa">Categoría</label><select name="categoria" class="form-control" style="background:#111!important;color:white!important"><option selected>{{p.categoria}}</option>{% for cat in categorias %}{% if cat.nombre!=p.categoria %}<option>{{cat.nombre}}</option>{% endif %}{% endfor %}</select></div><div class="col-md-3"><label class="label-rosa">Precio</label><input name="precio" type="number" step="0.01" class="crear-input" value="{{p.precio}}"></div><div class="col-md-3"><label class="label-rosa">Stock</label><input name="stock" type="number" class="crear-input" value="{{p.stock}}"></div></div><div style="margin-top:20px"><label class="label-rosa">Foto actual: <img src="{{p.img_url}}" style="width:80px;height:80px;object-fit:cover;border-radius:8px;background:white;padding:3px;border:1px solid var(--rosa)"></label><br><label class="label-rosa">Cambiar foto {% if cloudinary_enabled %}(Permanente ☁️){% endif %}</label><input name="imagen" type="file" class="form-control" accept="image/*"></div><button style="background:var(--rosa);color:white;border:none;padding:12px 30px;border-radius:6px;font-weight:bold;margin-top:20px">💾 GUARDAR CAMBIOS</button></div></form></div>""", p={'nombre':p.nombre,'categoria':p.categoria,'precio':p.precio or 0,'stock':p.stock or 0,'img_url':get_producto_imagen(p)}, categorias=categorias, cloudinary_enabled=CLOUDINARY_ENABLED)
...
def dueno_view():
    if not session.get('is_admin'): return redirect('/dashboard')
    return render_template_string(STYLE_BASE+nav()+"""<div class="container mt-3"><div class="card"><h4 style="color:var(--rosa)">💰 Dueño</h4><div style="display:flex;gap:10px;margin-top:15px"><select id="periodo" class="form-control" style="width:150px;background:#111!important;color:white!important" onchange="cargarGrafica()"><option value="dia">Día</option><option value="mes" selected>Mes</option><option value="ano">Año</option></select><button onclick="cargarGrafica()" class="btn-rosa">Actualizar</button></div><div style="background:#111;border:2px solid var(--rosa);border-radius:12px;padding:15px;margin-top:15px"><canvas id="graficaVentasGastos" height="100"></canvas></div><div class="row mt-3"><div class="col-md-4"><div class="card"><h6>Ventas</h6><h3 id="totalVentas" style="color:#25D366">$0</h3></div></div><div class="col-md-4"><div class="card"><h6>Gastos</h6><h3 id="totalGastos" style="color:var(--rosa)">$0</h3></div></div><div class="col-md-4"><div class="card"><h6>Ganancia</h6><h3 id="totalGanancia" style="color:var(--rosa)">$0</h3></div></div></div><div class="card mt-3"><h6>Gasto</h6><form id="formGasto" class="d-flex gap-2"><input id="concepto" class="form-control" placeholder="Concepto" required><input id="monto" type="number" step="0.01" class="form-control" placeholder="Monto" required><button class="btn-rosa">Agregar</button></form></div></div></div><script>let chart;function cargarGrafica(){let p=document.getElementById('periodo').value;fetch('/api/ventas_gastos?periodo='+p).then(r=>r.json()).then(data=>{document.getElementById('totalVentas').innerText='$'+data.total_ventas.toFixed(2);document.getElementById('totalGastos').innerText='$'+data.total_gastos.toFixed(2);document.getElementById('totalGanancia').innerText='$'+(data.total_ventas-data.total_gastos).toFixed(2);let ctx=document.getElementById('graficaVentasGastos').getContext('2d');if(chart) chart.destroy();chart=new Chart(ctx,{type:'line',data:{labels:data.labels,datasets:[{label:'Ventas',data:data.ventas,borderColor:'#25D366',tension:0.3},{label:'Gastos',data:data.gastos,borderColor:'#ff4d8a',tension:0.3}]}});});}document.getElementById('formGasto').addEventListener('submit',e=>{e.preventDefault();fetch('/api/gasto',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({concepto:document.getElementById('concepto').value,monto:document.getElementById('monto').value})}).then(()=>{cargarGrafica();});});cargarGrafica();</script>""")
...
def admin_config():
    ...
    logo_url=get_producto_imagen(cfg)
    return render_template_string(STYLE_BASE+nav()+"""<div class="container mt-4" style="max-width:600px"><div class="card"><h4>⚙️ Config + Logo {% if cloudinary_enabled %}<small style="background:#25D366;color:white;padding:3px 8px;border-radius:10px;font-size:10px">☁️ Activo</small>{% endif %}</h4><div class="text-center"><img src="{{logo_url}}" style="width:100px;height:100px;border-radius:50%;background:white;padding:5px;border:2px solid var(--rosa)"></div><form method="POST" enctype="multipart/form-data" class="mt-3"><label>Cambiar logo</label><input name="logo" type="file" class="form-control mb-3" accept="image/*"><div style="background:#0e0e0e;border:1px solid #222;border-radius:8px;padding:12px;margin-bottom:10px"><label style="font-size:13px"><input type="checkbox" name="mod_pos_mesero" {{'checked' if cfg.mod_pos_mesero}} style="accent-color:var(--rosa)"> POS para mesero</label><br><label style="font-size:13px;margin-top:10px"><input type="checkbox" name="mod_mesero_cobrar" {{'checked' if cfg.mod_mesero_cobrar}} style="accent-color:var(--rosa)"> <b>Permitir que mesero COBRE directo</b> (si no, solo solicita cuenta a caja)</label></div><button class="btn-rosa w-100 mt-2">Guardar</button></form></div></div>""", cfg=cfg, logo_url=logo_url, cloudinary_enabled=CLOUDINARY_ENABLED)
@app.route('/ticket/<int:id>')
def ticket(id):
    v=Venta.query.get(id)
    if not v: return redirect('/dashboard')
    entregado=request.args.get('entregado', type=float); cambio=request.args.get('cambio', type=float)
    return render_template_string(STYLE_BASE+f'<div class="container mt-5" style="max-width:380px"><div class="card" style="background:white;color:black"><h5 style="text-align:center">Ticket #{v.id}</h5><p style="font-size:13px"><b>{v.producto_nombre}</b><br>Total: {format_mxn(v.total)}<br>Pago: {v.metodo_pago.upper()}<br>Atendió: {v.vendedor_nombre}<br><small id="fechaPC" style="color:#888"></small></p>{"<div style=background:#0a1a0f;border:2px solid #25D366;border-radius:8px;padding:10px;margin:10px 0><div style=display:flex;justify-content:space-between><span>Entregado:</span><b>"+format_mxn(entregado)+"</b></div><div style=display:flex;justify-content:space-between><span>Cambio:</span><b style=color:#25D366;font-size:18px>"+format_mxn(cambio)+"</b></div></div>" if entregado is not None else ""}<button onclick="window.print()" class="btn-rosa w-100">Imprimir</button><a href="/dashboard" class="btn btn-dark w-100 mt-2">Volver POS</a></div></div><script>document.getElementById("fechaPC").textContent=new Date().toLocaleString("es-MX",{{timeZone:"America/Cancun"}})</script>')
@app.route('/ventas')
def ventas():
    if 'user' not in session: return redirect('/')
    is_admin=session.get('is_admin')
    vs=Venta.query.order_by(Venta.id.desc()).limit(200).all()
    return render_template_string(STYLE_BASE+nav()+"""<div class="container mt-3"><div class="card"><h5>🧾 Ventas</h5><table class="table table-dark table-sm mt-3" style="font-size:11px"><tr><th>ID</th><th>Fecha</th><th>Cliente</th><th>Producto</th><th>Total</th><th>Pago</th><th>Vendedor</th>{% if is_admin %}<th>Acción</th>{% endif %}</tr>{% for v in vs %}<tr><td>{{v.id}}</td><td>{{v.fecha.strftime('%d/%m %H:%M')}}</td><td>{{v.cliente}}</td><td>{{v.producto_nombre}} x{{v.cantidad}}</td><td style="color:#25D366">{{v.total_mxn}}</td><td>{{v.metodo_pago}}</td><td>{{v.vendedor_nombre}}</td>{% if is_admin %}<td><a href="/ventas/eliminar/{{v.id}}" style="color:var(--rosa)">Eliminar</a></td>{% endif %}</tr>{% endfor %}</table></div></div>""", vs=[{'id':v.id,'fecha':v.fecha,'cliente':v.cliente,'producto_nombre':v.producto_nombre,'cantidad':v.cantidad,'total_mxn':format_mxn(v.total),'metodo_pago':v.metodo_pago.upper(),'vendedor_nombre':v.vendedor_nombre or v.vendedor} for v in vs], is_admin=is_admin)
...
def admin_usuarios():
    if not session.get('is_admin'): return redirect('/dashboard')
    if request.method=='POST':
        username=request.form.get('username','').strip(); nombre=request.form.get('nombre_completo','').strip(); pwd=request.form.get('password','').strip(); rol=request.form.get('rol','cajero'); is_admin='is_admin' in request.form
        if username and nombre and pwd and not User.query.filter_by(username=username).first():
            if is_admin: rol='admin'
            db.session.add(User(username=username,nombre_completo=nombre,password=generate_password_hash(pwd),is_admin=is_admin,rol=rol)); db.session.commit()
